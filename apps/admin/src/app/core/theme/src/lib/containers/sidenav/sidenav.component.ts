...
export class SidenavComponent implements OnInit, OnDestroy {
  ...
  getSubscriptionPlan(config) {
    this.$subscription.add(
      this._globalFilterService.globalFilter$.subscribe((data) => {
        ...
        this.subscriptionPlanService.getSubscriptionPlan(hotelId).subscribe((response) => {
          this.subscriptionPlanService.setSubscription(response.featuresIncludes);
  ...
  private initSideNavConfigs(subscription, config = {}) {
    ...
    ADMIN_ROUTES.forEach((data) => {
      if (subscription.featuresIncludes.filter((d) => d.featureName === data.title && d.isView).length) {
