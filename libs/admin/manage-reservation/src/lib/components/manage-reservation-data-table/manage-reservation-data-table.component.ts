...
import { GlobalFilterService, RoutesConfigService, SubscriptionPlanService } from '@hospitality-bot/admin/core/theme';
import {
    AdminUtilityService,
...
import { BookingConfig, ReservationFormData, ReservationList, RoomReservation } from '../../models/reservations.model';
import { ManageReservationService } from '../../services/manage-reservation.service';
import { ReservationListResponse, RoomReservationResponse } from '../../types/response.type';
...
export class ManageReservationDataTableComponent extends BaseDatableComponent {
    ...
    initTableValue() {
        ...
        this.manageReservationService
            .getReservationItems<ReservationListResponse>(this.getQueryConfig(), this.entityId)
            .subscribe(
                (res) => {
     ...
     */
    handleStatus(status: ReservationStatusType, reservationData: RoomReservation): void {
        let modalRef: DynamicDialogRef;

        const data = {
            content: {
                heading: `Mark Reservation As ${status.charAt(0).toUpperCase() + status.slice(1).toLowerCase()}`,
                descriptions: [
    ...
    changeStatus(status: ReservationStatusType, reservationData: RoomReservation, additionalData?: any) {
        this.loading = true;
        this.$subscription.add(
            this.manageReservationService
                .updateBookingStatus(reservationData.id, this.entityId, EntitySubType.ROOM_TYPE, {
                    reservationType: status,
                    remarks: additionalData?.remarks
                })
                .subscribe(
                    (res) => {
                        this.values.find((item) => item.id === reservationData.id).reservationType = status;
                        this.initTableValue();
                        this.snackbarService.openSnackBarAsText('Reservation ' + status + ' changes successfully', '', {
                            panelClass: 'success'
                        });
                    },
                    this.handleError,
    ...
    cloneReservation(reservationId: string) {
        const selectedReservation = this.roomReservationList.find((item) => item.id === reservationId);
        const reservation = new ReservationFormData().deserialize(selectedReservation);
        const guestName =
            reservation.guestInformation?.firstName || reservation.guestInformation?.lastName
                ? reservation.guestInformation?.firstName + ' ' + (reservation.guestInformation?.lastName ?? '')
                : '';
