...
export class RequestDetailComponent implements OnInit, OnDestroy {
    ...
    listenForGlobalFilters(): void {
        this.$subscription.add(
            this.globalFilterService.globalFilter$.subscribe((data) => this.globalFilterService.entityId)
        );
    }
            ...
            this._requestService.updateJobRequestStatus(this.entityId, config, requestData).subscribe(
                (response) => {
                    this.snackbarService.openSnackBarAsText(
                        `Job: ${this.data.jobNo} status updated successfully to ${convertToTitleCase(event.value)}.`,
                        '',
                        { panelClass: 'success' }
                    );
    ...
    formattedDate() {
        const dateObject: Date = this.closedTimestamp ? new Date(this.closedTimestamp) : new Date();
        this.formattedClosedTimestamp = this.datePipe.transform(dateObject, "EEEE, MMMM d, y, 'at' h:mm a");
    }
    ...
    handleAssigneeChange(event) {
        this._requestService
            ...
            .subscribe(() => {
                const user = this.assigneeList.find((item) => item.value === event.value)?.label ?? '';
