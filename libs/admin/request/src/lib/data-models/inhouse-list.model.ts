...
export class InhouseTable {
    ...
    deserialize(input) {
        ...
        input?.records?.forEach((record) => this.records.push(new InhouseData().deserialize(record)));
...
export class InhouseData {
    ...
    getRequestDateTime(timezone = '+05:30') {
        return `${DateService.getDateFromTimeStamp(
            ...
            'D-M-YYYY',
            timezone
        )} at ${DateService.getDateFromTimeStamp(this.requestTime, 'h:mm a', timezone)}`;
    }
    ...
    getClosedTime(timezone = '+05:30') {
        if (this.closedTime)
            return `${DateService.getDateFromTimeStamp(
                ...
                'D-M-YYYY',
                timezone
            )} at ${DateService.getDateFromTimeStamp(this.closedTime, 'h:mm a', timezone)}`;
        else '------';
    }
    ...
    getSLAvalue() {
        if (this.elapsedTime) return Math.round(((this.elapsedTime % 86400000) % 3600000) / 60000);
        else 0;
    }
...
export class GuestType {
    ...
    getSecGuestDisplayConfig() {
        return {
            ...
            countString: this.secondaryGuest.length > 0 ? `(+${this.secondaryGuest.length})` : null
        };
    }
...
export class Guest {
    ...
    deserialize(input: any) {
        Object.assign(
            ...
            set({}, 'countryCode', this.getNationality(get(input, ['contactDetails', 'cc']))),
    ...
    getPhoneNumber() {
        return this.phoneNumber ? `${this.countryCode ? this.countryCode : ''} ${this.phoneNumber}` : '';
    }
