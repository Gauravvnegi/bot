...
import { ModuleNames, NavRouteOption, Option } from '@hospitality-bot/admin/shared';
...
export class CreateServiceItemComponent implements OnInit {
    ...
    constructor(
        ...
        this.serviceItemId = this.route.snapshot.paramMap.get(parmaId.editServiceItem);
    ...
    listenForFormValuesChanges() {
        ...
        defaultItemUser.valueChanges.subscribe((newValue) => {
            ...
            const filteredValue = currentValue.length && currentValue?.filter((value) => value !== newValue);
    ...
    getServiceItemDetails() {
        this.$subscription.add(
            this.serviceItemService.getServiceItemById(this.entityId, this.serviceItemId).subscribe((res) => {
                this.serviceItemForm.patchValue(new ServiceItemFormData().deserialize(res));
            })
        );
    }
            ...
            this.managePermissionService
                ...
                .subscribe((data) => {
                    this.userList = data.users.map((item) => ({
                        ...
                        value: item.id,
                        extras: item?.departments.map((item) => convertToNormalCase(item.department)).join(', ')
                    }));
                })
        );
    }
    getCategoryList() {
        this.$subscription.add(
            this.serviceItemService.getCategoryList(this.entityId, { params: '?limit=0&offset=0' }).subscribe((res) => {
    ...
    handleSubmit() {
        if (this.serviceItemForm.invalid) {
            ...
            this.snackbarService.openSnackBarAsText('Invalid form: Please fix the errors.');
            return;
        }
        ...
        this.serviceItemService
            ...
            .subscribe(
                (res) => {
                    ...
                    this.snackbarService.openSnackBarAsText('Category created successfully', '', {
                        panelClass: 'success'
                    });
                },
                () => {}
    ...
    handleSuccess = (res) => {
        ...
        this.snackbarService.openSnackBarAsText(
            this.serviceItemId ? 'Service Updated successfully' : `Service Created successfully`,
            '',
            { panelClass: 'success' }
        );
    ...
    get userOptions() {
        return this.userList?.filter((user) => user?.value !== this.serviceItemForm?.get('defaultItemUser')?.value);
    }

    get serviceItemFormControls() {
        return this.serviceItemForm.controls as Record<keyof ServiceItemFormData, AbstractControl>;
    }
