...
export class TimePickerComponent extends FormComponent {
    ...
    constructor(public fb: FormBuilder, public controlContainer: ControlContainer) {
    ...
    ngOnInit(): void {
        ...
        this.$subscription = interval(1000).subscribe(() => {
            if (this?.inputControl?.touched) {
                this.timePickerFG.markAllAsTouched();
            }
        });
    }
    ...
    listenChanges() {
        ...
        this.controlContainer.control.get(this.controlName).valueChanges.subscribe((value) => {
            if (value) {
                const [HH, MM] = (this.format === 'HH:MM' ? value : this.millisecondsToRequiredFormat(+value)).split(
                    ':'
                );
        ...
        this.timePickerFG.valueChanges.subscribe((value: { hh: string; mm: string }) => {
    ...
    handleKeyPress(event: Event, type: 'HH' | 'MM') {
        ...
        if ((isNumber && type === 'HH' && value > 23) || (type === 'MM' && value > 59)) {
