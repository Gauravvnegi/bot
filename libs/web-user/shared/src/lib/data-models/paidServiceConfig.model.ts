...
export class PaidServiceDetailDS implements Deserializable {
  ...
  deserialize(input: any, selectedAmenities: any, arrivalTime: string) {
    ...
    this.paidService = this.updateAmenities(
      this.paidService,
  ...
  updateAmenities(paidService, selectedService) {
    this.paidService = new PaidServiceDetail().checkForSelectedAmenity(
      paidService,
      selectedService
    );
        ...
        if (paidAmenity.subPackages?.length > 0) {
          paidAmenity.subPackages.forEach((subPackage, subPackageIndex) => {
            if (subPackage.packageCode === selectedAminity.packageCode) {
              ...
              paidAmenity.subPackages[subPackageIndex].quantity =
                selectedAminity.quantity;
              paidAmenity.subPackages[subPackageIndex].metaData =
                selectedAminity.metaData;
            }
          });
        }
      });
